  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="md:hidden mb-3">
        <button
          onClick={() => setOpen((s) => !s)}
          className="w-full flex items-center justify-between px-3 py-2 bg-white/5 border border-white/10 rounded-lg"
        >
          <div className="flex items-center gap-3">
            <Menu className="w-5 h-5 text-white" />
            <span className="text-sm font-medium text-white">Add Fields</span>
          </div>
          <div className="text-slate-400">{open ? "Hide" : "Show"}</div>
        </button>
      </div>

      <div className={`${open ? "block" : "hidden"} md:block`}>
        <h2 className="text-lg font-bold text-white mb-6 hidden md:block">Add Fields</h2>
                ...
                {group.fields.map((field) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full p-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-purple-500/50 rounded-lg transition-all group text-left flex items-center gap-3 touch-manipulation"
