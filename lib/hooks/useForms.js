...
export function useForms() {
  ...
  const handleCreateForm = async (formData) => {
    try {
      const created = await createForm(user.uid, formData);

      // createForm returns the new document id. Fetch the full object
      // so we always store form objects in state (prevents mixed types)
      let newForm = created;
      if (typeof created === "string") {
        newForm = await getFormById(created);
      }
